...
def run_react_agent(user_query: str, provider, mcp_server: MCPHelpdeskServer) -> list:
    """
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            if not obs_data:
                ...
                if obs_data.get("status") == "SUCCESS":
                    if "data" in obs_data:
                        final_answer = f"Kết quả Helpdesk: {json.dumps(obs_data['data'], ensure_ascii=False)}"
                    elif "message" in obs_data:
                        final_answer = obs_data["message"]
                    else:
                        final_answer = f"Đã hoàn tất xử lý qua MCP Server: {json.dumps(obs_data, ensure_ascii=False)}"
                elif obs_data.get("status") == "NOT_FOUND":
                    final_answer = obs_data.get("message", "Không tìm thấy thông tin Helpdesk yêu cầu.")
                else:
            ...
            # Cho phép luồng Helpdesk kiểm tra ticket rồi mới tạo yêu cầu tiếp theo.
            if (
                tool_name == "helpdesk_query"
                and obs_data.get("status") == "SUCCESS"
                and "kiểm tra ticket" in original_query.lower()
                and "tạo yêu cầu mới" in original_query.lower()
            ):
                user_query = (
                    f"{original_query}\nObservation: ticket đang ở trạng thái "
                    f"{obs_data.get('data', {}).get('status', 'chưa xử lý')}. "
                    "Hãy tạo yêu cầu mới."
                )
                continue
...
if __name__ == "__main__":
    ...
    if "--interactive" in sys.argv:
        ...
        print("   - Câu hỏi chung: 'Tôi nên làm gì khi không vào được Wi-Fi?'")
        print("   - Tra cứu ticket: 'Hãy tra cứu ticket IT-1001'")
        print("   - Tạo yêu cầu: 'Tạo ticket vì tôi không đăng nhập được tài khoản'")
